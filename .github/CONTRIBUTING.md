...
Review and merge that pull request to create the matching GitHub prerelease. Check the tag, changelog, and prerelease contents, then edit the release, clear **Set as a pre-release**, and update it to promote the release. That promotion publishes the tagged version to npm.

Neither release workflow pushes directly to `main`. The prerelease workflow can create the tag and GitHub release but has no registry identity. The publish workflow has registry identity but read-only repository access.
